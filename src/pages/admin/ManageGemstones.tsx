                ...
                {isEditing ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {gemstones.map((gemstone) => (
                            <div key={gemstone._id} className="bg-white rounded-3xl shadow-lg p-5 flex flex-col gap-4" style={{ minHeight: "520px" }}>
                                <div className="aspect-square rounded-2xl overflow-hidden bg-gray-100 relative group">
                                    <img src={gemstone.image} alt={gemstone.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                                </div>
                                <div>
                                    <h3 className="font-semibold text-xl text-gray-900">{gemstone.name}</h3>
                                    <p className="text-sm text-gray-500 line-clamp-2 mt-1">{gemstone.shortDescription}</p>
                                    <p className="font-bold mt-2 text-lg text-gray-900">{gemstone.price}</p>
                                </div>
                                <div className="flex gap-2 mt-auto">
                                    <button onClick={() => handleEdit(gemstone)} className="flex-1 px-3 py-2 bg-secondary text-foreground rounded-lg hover:bg-muted flex items-center justify-center gap-2 transition-colors">
                                        <Edit className="w-4 h-4" /> Edit
                                    </button>
                                    <button onClick={() => handleDelete(gemstone._id)} className="flex-1 px-3 py-2 bg-destructive/10 text-destructive rounded-lg hover:bg-destructive/20 flex items-center justify-center gap-2 transition-colors">
